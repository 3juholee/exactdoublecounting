    ...
    ******************************************************************************/
    ...
    def Ec(self, rs): # Ec-Vc
        if self.type<3 :
            ...
            x = sqrt(rs)
            return self.Ap/6.*(self.cp*(x-self.xp0)-self.bp*x*self.xp0)/((x-self.xp0)*(x*x+self.bp*x+self.cp))+self.Vc(rs)
        else:
            if rs>1:
                return 2*self.gamma/(1+self.beta1*sqrt(rs)+self.beta2*rs)
            else:
                return self.Aw*log(rs)+self.Bw+self.Cw*rs*log(rs)+self.D*rs
