...
def Plmc(x,m,maxk):
    ...
    codep="""
      long double xl = x;
      long double Pm0 = Plm(0);
      long double Pm1 = Plm(1);
      for (int l=m+1; l<m+maxk-1; l++){
          long double c = l-m+1.;
          long double beta = (2*l+1.)*xl/c;
          long double gamma = (l+m)/c;
          long double Pm2 = beta*Pm1-gamma*Pm0;
          Plm(l-m+1) = Pm2;
          Pm0 = Pm1;
          Pm1 = Pm2;
      }
...
def old_Laguerre_dirty(x,alpha,max_n):
    ...
    codel="""
       ...
       if (max_n >=0 ){
       long double ln2 = 1.0L;
       long double ln1 = alpha_one_mx;
       Lag(0) = (double) ln2;
       if (max_n > 0){
       Lag(1) = (double) ln1;
       if (max_n > 1){
       ...
       for (int k = 1; k < max_n; k++) {
          long double beta = ( (long double)(k + k) + alpha_one_mx );
          long double gamma = (long double) k + (long double) alpha;
          long double ln = (beta * ln1 - gamma * ln2) / (long double)(k+1);
          Lag(k+1) = (double)ln;
          ln2 = ln1;
          ln1 = ln;
       }
